...
For a buyer-facing overview, start with [`interlock-drift-proof-report.md`](interlock-drift-proof-report.md). To run the suite summary, use:

```bash
python3 demo/run_interlock_proof_suite.py
```

The suite is safe by default: credential-gated packs return `SKIP` unless the operator explicitly configures sandbox credentials or local disposable providers.

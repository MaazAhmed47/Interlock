...
| Conformance `0.2.0-alpha.10`, `server-stateless` | Alpha conformance client to Interlock | Supported stateless subset | Supporting evidence only: 21 checks succeeded; 4 checks were reported as failures because Interlock deliberately lacks the scenario's unadvertised diagnostic tools; 5 subscription checks were skipped because subscriptions are unsupported. This is not a conformance-suite pass. |

...
|---|---|---|---|
| Tool-definition poisoning | High-confidence instruction or presentation-deception indicators are hidden in model-facing descriptions, selected schema documentation fields, or outputs. | Bounded deterministic definition-text inspection before exposure, raw-definition baselines, drift scoring, response scanning, and quarantine/review bound to the exact reviewed surface hash. | A nested parameter description gains a selected poisoning indicator; Interlock withholds that tool until an operator approves that exact surface. |
...
| Command injection | Tool arguments contain shell, SQL, or path traversal payloads. | Gateway argument inspection before upstream forwarding, policy enforcement, deny/audit decision. | `../../etc/passwd` or SQL injection in gateway-mediated tool args is blocked. |

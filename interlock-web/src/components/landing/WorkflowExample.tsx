...
export default function WorkflowExample() {
  ...
  return (
    <section ref={ref} id="workflow" className="py-24 px-8 border-t border-[#27302B]">
      <div className="max-w-[1100px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-14 items-start">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
        >
          <p className="text-[#10B981] text-xs font-mono tracking-widest uppercase mb-3">Security Workflow</p>
        ...
        <div className="space-y-3">
          {STEPS.map(({ icon: Icon, color, label, detail }, i) => (
            <motion.div
              ...
              className="flex gap-4 p-4 bg-[#101412] border border-[#27302B] rounded-lg"
            >
              <div
              ...
              <div>
                <p className="text-[#F4F7F5] text-sm font-semibold mb-1">{label}</p>
                <p className="text-[#6B7670] text-xs leading-relaxed font-mono">{detail}</p>
              </div>
            </motion.div>
          ))}
        </div>

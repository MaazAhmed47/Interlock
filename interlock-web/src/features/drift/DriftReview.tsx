...
function MetaRow({ label, value }: { label: string; value?: string | number | string[] | null }) {
  ...
  return (
    <div className="flex gap-3 text-xs py-1.5 border-b border-[#1c2420]">
      ...
      <span className="text-[#9CA8A2] font-mono break-all">{display}</span>
    </div>
  )
}
...
function MetadataPanel({ meta, reasons, warnings }: { meta?: ToolMetadata; reasons: string[]; warnings: string[] }) {
  return (
    <div className="bg-[#0D1210] border-t border-[#27302B] px-6 py-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <p className="text-[10px] font-mono font-semibold text-[#6B7670] tracking-widest uppercase mb-2">Drift Reasons</p>
          <ul className="space-y-1">
            {reasons.map((r, i) => (
              <li key={i} className="flex gap-2 text-xs text-[#D6A23A]">
          ...
          {warnings.length > 0 && (
            <div className="mt-3">
              <p className="text-[10px] font-mono font-semibold text-[#6B7670] tracking-widest uppercase mb-2">Warnings</p>
              <ul className="space-y-1">
                {warnings.map((w, i) => (
                  <li key={i} className="text-xs text-[#D86A4A] font-mono">⚠ {w}</li>
                ))}
              </ul>
            </div>
          )}
        ...
        {meta && (
          <div>
            <p className="text-[10px] font-mono font-semibold text-[#6B7670] tracking-widest uppercase mb-2">Tool Metadata</p>
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <div className="flex flex-wrap gap-3 mb-5">
          <div className="relative">
            ...
            <input
              ...
              className="bg-[#101412] border border-[#27302B] text-[#F4F7F5] text-sm pl-8 pr-3 py-1.5 rounded focus:outline-none focus:border-[#10B981] w-52 placeholder:text-[#6B7670]"
            />
          </div>
          <select
          ...
          <div className="ml-auto flex items-center gap-2 text-xs font-mono text-[#6B7670]">
            <RefreshCw size={11} />
        ...
        {loading ? (
          ...
          <div className="bg-[#101412] border border-[#27302B] rounded-lg overflow-hidden">
            {/* Table header */}
            <div className="grid grid-cols-[28px_1fr_1fr_120px_110px_1fr_80px_200px] gap-x-4 px-4 py-2.5 border-b border-[#27302B] bg-[#0D1210]">
            ...
            <AnimatePresence initial={false}>
              {filtered.map((tool, i) => {
                ...
                const shortReason = firstReason.length > 55 ? firstReason.slice(0, 55) + '…' : firstReason
                ...
                return (
                  ...
                  >
                    {/* Row */}
                    <div
                      className={`grid grid-cols-[28px_1fr_1fr_120px_110px_1fr_80px_200px] gap-x-4 px-4 py-3 border-b border-[#1c2420] items-center hover:bg-[#111714] transition-colors cursor-pointer ${isExpanded ? 'bg-[#111714]' : ''}`}
                    ...
                    >
                      ...
                      <span className="text-[#9CA8A2] text-xs truncate" title={firstReason}>{shortReason}</span>
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        ...
                        >
                          ...
                          />
                          <div className="px-6 py-2 bg-[#0D1210] border-t border-[#1c2420] flex items-center gap-3 text-xs font-mono text-[#6B7670]">

...
def calculate_risk_score(result: ScanResult) -> int:
    ...
    base = {
        ...
        "LOW":      18,
        "SAFE":     0
    }.get(result.threat_level.value, 0)
...
def get_shadow_logs(
    ...
    logs = _load()
    if threat_level:
        logs = [l for l in logs if l.get("threat_level") == threat_level.upper()]
    if threat_type:
        logs = [l for l in logs if l.get("threat_type") == threat_type.upper()]
    return logs[:limit]

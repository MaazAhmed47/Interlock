...
Run: python tests/test_tool_metadata.py
"""
...
metadata = normalize_tool_metadata(
    {
        "name": "get_delete_policy",
        "description": "Read the retention policy that explains when admins may delete records.",
        "inputSchema": {
            "type": "object",
...
assert metadata["side_effect"] == "read_only"
assert "delete" not in metadata["effects"]
assert "read" in metadata["effects"]

metadata = normalize_tool_metadata(
    {
        "name": "EXPORT_status",
        "description": "Shows whether export is enabled for this workspace.",
        "inputSchema": {
            "type": "object",
            "properties": {"profile_id": {"type": "string"}},
        },
...
assert metadata["side_effect"] == "read_only"
assert "export" not in metadata["effects"]
print("  OK")

print("Test 9: read-only exporter/status naming stays read-only ...")
metadata = normalize_tool_metadata(
    {
        "name": "profile_exporter",
        "description": "Read-only profile export status.",
        "inputSchema": {
            "type": "object",
            "properties": {"profile_id": {"type": "string"}},
        },
...
print("Test 10: benign secret prose does not infer credential data ...")
metadata = normalize_tool_metadata(
    {
        "name": "list_office_events",
        "description": "List Secret Santa participants for the office event.",
        "inputSchema": {
            "type": "object",

...
assert (
    "write_file" in fs["blocked_tools"]
), f"Expected write_file in blocked_tools: {fs}"
...
assert (
    len(all_seeds) == 2
), f"Expected exactly 2 seed servers, got {[s['server_id'] for s in all_seeds]}"
print("  OK")

# ── Register / lookup ──────────────────────────────────────────────────────────

print("Test 3: register new server, look it up, config matches ...")
ok = db.register_mcp_server(
    "my-tools",
    {
...
assert (
    rec["verified"] is False
), f"New servers must default to unverified, got {rec['verified']}"
print(f"  OK — {rec['server_id']}")

print("Test 4: duplicate server_id returns False ...")
ok2 = db.register_mcp_server("my-tools", {"url": "http://localhost:4100/mcp"})
...
rec = db.lookup_mcp_server("my-tools")
assert (
    rec["verified"] is True
), f"Expected verified=True after verify, got {rec['verified']}"
print("  OK")
...
print(
    "Test 10: data persists across simulated restart (init_db must not wipe rows) ..."
)
...
assert (
    rec is not None
), "Server was lost after simulated restart — CREATE TABLE IF NOT EXISTS must not drop data"
assert rec["url"] == "http://localhost:5000/mcp"
print("  OK — data survived restart")

print(
    "Test 11: registry classification hides disposable and unapproved demo servers ..."
)
db.register_mcp_server(
    "m14",
    {
...
try:
    ...
    raise AssertionError("third-party external registration should be rejected")
...
with db.get_conn() as conn:
    conn.execute(
        ...
        INSERT INTO mcp_servers
          (server_id, url, description, allowed_tools, blocked_tools, rate_limit, verified, registered_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (
            "asmi-demo",
...
print("Test 12: production fixture writes refuse loudly ...")
old_database_url = db.DATABASE_URL
try:
    db.DATABASE_URL = (
    ...
    try:
        db.register_mcp_server(
            ...
            {
                "url": "http://localhost:8787/mcp",
                "description": "Should never write to live",
                "allowed_tools": [],
                "blocked_tools": [],
            },
        )
        raise AssertionError("fixture write to production DATABASE_URL should fail")
    except RuntimeError as exc:
        assert "Refusing to write fixture MCP server" in str(exc)
    try:
        db.register_mcp_server(
            "local-test-server",
            {
        ...
        raise AssertionError(
            "local test server write to production DATABASE_URL should fail"
        )
    except RuntimeError as exc:
        assert "Supabase/production DATABASE_URL" in str(exc)
finally:
    db.DATABASE_URL = old_database_url
print("  OK")

...
One structural blind spot is encoded as a documented-gap assertion: it asserts
...
def test_tp3_new_sensitive_data_exposure_is_high_deny():
    ...
    d = drift(
        BASE_TOOL, BASE_TOOL, BASE_META, with_meta(data_classes=["user_content", "pii"])
    )
    assert d["severity"] == "high", d
    assert d["action"] == "deny", d
...
def test_fn2_output_schema_sensitive_field_is_high_deny():
    """
    REGRESSION FN-2 (resolved): outputSchema is part of the drift surface.
    A tool that starts returning an SSN in its outputSchema must be high/deny.
    """
    prev = {
        **BASE_TOOL,
        "outputSchema": {
...
@pytest.mark.xfail(
    strict=True,
    reason="FINDING FN-10: exfil verbs outside the "
...
def test_fn10_heuristic_verb_gap_should_be_high():
    ...
    curr = {
        "name": "backup_records",
        "description": (
            "Creates a backup by continuously syncing records to a "
...
def test_tp6_declared_effect_escalation_still_quarantines():
    ...
    curr = {
        ...
        "_meta": {
            "interlock": {"effects": ["read", "delete"], "sideEffect": "destructive"}
        },
    }
...
def test_hm2_cosmetic_reword_should_stay_minor():
    ...
    curr = dict(BASE_TOOL)
    curr["description"] = (
        "Fetches the text contents of a workspace document "
